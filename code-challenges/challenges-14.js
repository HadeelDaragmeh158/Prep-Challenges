'use strict';

// Resource:
// split: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/split
// slice: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/slice
// splice: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/splice
// indexOf: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/indexOf
// lastIndexOf: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/lastIndexOf

// 1) ---------------------
// 
// Using the slice and indexOf/lastIndexOf methods, return the last word in a string
//
// Note: consider that the string may have one word but never empty
//
// Ex: "I did my waiting 12 years of it in Azkaban" ==> "Azkaban"
// Ex: "Impossible" ==> "Impossible"
//
// ------------------------

const LastWord = (str) => {
    // write your code here
    let x= str.lastIndexOf(' ');
    if(x>-1)
        return str.slice(x+1);
    if (x==-1) {
        x=str.slice(0);
        return x;}
    
}

// 2) ---------------------
// 
//  Hopefully you struggled in the last code challenge, you can try to do the same task using the split method
//
//  EX: "you dare use my spells against me, potter" ===> "potter"
//
// ------------------------

const LastWord_2 = (str) => {
    // write your code here
    let x=str.split(' ');
    let y=x[x.length-1];
   let r= y.split('');//
   if (r[0]==' ')
   r=r.slice(1).join ("");
   else r=r.join ("");
   return  (r);
}

// 3) ---------------------
// 
// Number 2 was easy right?, ok then using the splice method replace all the "I" with "We", "am" with "are" and "was" with "were"
//
// Note: you may use another method for search (indexOf)
//
// Ex: "I thought you had a plan" ==> "We thought you had a plan"
// Ex: "I was there 3000 years ago" ==> "We were there 3000 years ago"
// Ex: "I am Venom" ==> "We are Venom"
//
// ------------------------

const replaceWords = (str) => {
    // write your code here
    let arr = str.split(' ');
    let stri=[];let stram=[];let strwas=[];
   
    let i= arr.indexOf('I');
      let am = arr.indexOf ('am');
      let was = arr.indexOf('was');
      // console.log (i,am,was);
      if (i>-1 && am >-1 && was>-1){
       stri = arr.splice(i,1,'We');
          stram=stri.splice(am,1,'are');
          strwas=stram.splice(was,1,'were');
      }
      else if( !(i>-1)){
       stram=arr.splice(am,1,'are');
          strwas=stram.splice(was,1,'were');
            return strwas.join ("")
          }
         else if( !(am>-1)){  
            stri = arr.splice(i,1,'We');
               strwas=stri.splice(was,1,'were');
               return strwas.join ("");
         }
           else if( !(was>-1)){  
            stri = arr.splice(i,1,'We');
          stram=stri.splice(am,1,'are');
          return stram.join ("");
        }
           }
// 4) ---------------------
// 
// Write a function that "joins" the array of words together and put a comma "," after every five words
// ["move","it","away","from","the","street"] ==> "move it away from the, street" 
//
// ------------------------

const arrToStr = (arr) => {
    // write your code here
    let arr2=arr.slice(0,5),arr3=[],arr4=[],arr5=[];
    if (arr.length>5)
    {
         arr3=arr.slice(6);
    }
  arr4=`${arr2},${arr3}`;
  arr5=arr4.splice(5,0,',');
  let string=arr5.join("");
  console.log(string);
}

// 5) ---------------------
// 
// Simon got a string manipulation question for his interview, it asked him to replace the duplicated letters in a string to an indicator and counter
// as in the example:
//
// "aaaa bbb sdf" ===> "a4 b3 s1d1f1"
//  "door" ==> "d1o2r1"
//
// help him by writing the function
//
// ------------------------

const letterCounter = (str) => {
    // write your code here
    let count = 0;
    for(let i = 0; i < str.length; i++){
       if(i === str.lastIndexOf(str[i])){
         str.splice(i,1,i);
       };
       count++;
    };
    return count;
 
}



module.exports = { LastWord, LastWord_2, replaceWords, arrToStr, letterCounter };